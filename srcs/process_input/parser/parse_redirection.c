...
/*   Updated: 2025/02/11 13:12:14 by nlouis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*	- returns: `0` on success, `-1` if a syntax error occurs.
*/
static int	parse_redirect_output(t_ms *ms, t_cmd *cmd, t_tk **tks, int *i)
{
	if (!tks[*i + 1])
	{
		free_cmd(cmd);
	...
	cmd->output_redirect = ft_strdup(tks[*i + 1]->value);
	if (!cmd->output_redirect)
	{
		free_cmd(cmd);
		error(ms, "ft_strdup(): malloc failed");
...
int	parse_redirections(t_ms *ms, t_cmd *cmd, t_tk **tks, int *i)
{
	...
	if (tks[*i]->type == TK_REDIRECT_OUTPUT)
		return (parse_redirect_output(ms, cmd, tks, i));
	if (tks[*i]->type == TK_APPEND_OUTPUT)
		return (parse_redirect_output(ms, cmd, tks, i));
